...
VIDEO_R1_BASE = Path("/dss/mcmlscratch/06/di38riq/experiment_logs/Video/Video-R1-7B")
EXISTING_ANALYSIS = Path("/dss/dsshome1/06/di38riq/rl_multi_turn/analysis")
OUTPUT_DIR = EXISTING_ANALYSIS / "video_r1"
...
COLORS = {
    ...
    "method": ["#443acf", "#bd0a0a", "#ff8c00"],  # Sequential, Video-Qwen, Video-R1
...
def plot_accuracy_vs_frames(all_stats, output_dir):
    ...
    ax2.plot(frames, mras, "s-", color="#e15759", linewidth=2.5, markersize=10,
             label="Video-R1-7B (MRA)", zorder=3)
    for x, y in zip(frames, mras):
        ax2.annotate(f"{y:.1f}%", (x, y), textcoords="offset points",
                     xytext=(0, 12), ha="center", fontsize=10, fontweight="bold")
    ax2.set_xlabel("Number of Frames")
    ax2.set_ylabel("MRA (%)")
    ax2.set_title("Video-R1-7B: MRA vs Frames (Numerical Questions)")
...
def plot_category_accuracy(all_stats, output_dir):
    ...
    ax.set_title("Video-R1-7B: Accuracy by Question Type and Frame Count")
    ax.set_xticks(x)
...
def plot_category_mra(all_stats, output_dir):
    ...
    ax.set_xlabel("Question Type")
    ax.set_ylabel("MRA (%)")
    ax.set_title("Video-R1-7B: MRA by Numerical Question Type")
    ax.set_xticks(x)
...
def plot_dataset_breakdown(all_stats, output_dir):
    ...
    ax.set_ylabel("Accuracy (%)")
    ax.set_title("Video-R1-7B: Accuracy by Dataset")
    ax.set_xticks(x)
...
def plot_no_answer_rate(all_stats, output_dir):
    ...
    ax.set_title("Video-R1-7B: No-Answer Rate by Question Type")
    ax.set_xticks(x)
...
def plot_overall_combined(all_stats, output_dir):
    ...
    plt.suptitle("Video-R1-7B on VSI-Bench: Overview", fontsize=16, fontweight="bold", y=1.01)
    plt.tight_layout()
...
def plot_comparison_with_qwen(all_stats, output_dir):
    """Compare Video-R1-7B against existing Qwen3-VL-8B Video and Sequential results."""
    ...
    plt.suptitle("Video-R1-7B vs Qwen3-VL on VSI-Bench", fontsize=16, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def plot_answer_breakdown(all_dfs, output_dir):
    ...
    plt.suptitle("Video-R1-7B: Answer Breakdown", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def plot_heatmap(all_stats, output_dir):
    ...
    ax.set_title("Video-R1-7B: Accuracy Heatmap (Frames vs Question Type)")
...
def main():
    ...
    for n_frames in FRAME_CONFIGS:
        ...
        # Save aggregated CSV
        agg_path = EXISTING_ANALYSIS / f"VideoR1_7B_{n_frames}f_aggregated.csv"
    ...
    # 3. Save performance summary CSV (append Video-R1 rows)
    summary_rows = []
    for s in all_stats:
