...
FILETYPES_TEST = ['.sens', '.txt', '_vh_clean.ppython analysis/scripts/get_scannet.py --scene_list scenes.txt --type _vh_clean_2.ply -o /dss/mcmlscratch/06/di38riqpython analysis/scripts/get_scannet.py --scene_list scenes.txt --type _vh_clean_2.ply -o /dss/mcmlscratch/06/di38riqpython analysis/scripts/get_scannet.py --scene_list scenes.txt --type _vh_clean_2.ply -o /dss/mcmlscratch/06/di38riqly', '_vh_clean_2.ply']
...
def main():
    ...
    parser.add_argument('--scene_list', help='path to text file containing scene ids to download (one per line)')
    ...
    if args.task_data:  # download task data
    ...
    elif args.scene_list:  # download scenes from file
        with open(args.scene_list, 'r') as f:
            scene_ids = [line.strip() for line in f if line.strip()]
        
        print(f'Downloading {len(scene_ids)} scenes from {args.scene_list}...')
        for scan_id in scene_ids:
            ...
            if scan_id not in release_scans and (not is_test_scan or args.v1):
                print('WARNING: Skipping invalid scan id: ' + scan_id)
                continue
        ...
        print(f'Completed downloading {len(scene_ids)} scenes.')
