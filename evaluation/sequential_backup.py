...
"""
    ...
    # Build message with LABELED images - each image preceded by its angle label
    content = [{"type": "text", "text": prompt_intro}]
    
    for angle in [0, 90, 180, 270]:
        img_path = view_image_paths.get(angle)
        if img_path:
            # Add label BEFORE each image so the model knows which angle it is
...
def load_mesh_cached(mesh_path):
    ...
    if mesh_path_str not in _mesh_cache:
        ...
        # Limit cache size to avoid memory issues (keep last 5 meshes)
        if len(_mesh_cache) > 5:
...
- Mentally place yourself at that position and orientation"""
        ...
        # Clearer explanation of directions
        direction_note = """
**Direction Reference:**
- Imagine standing at the viewpoint and facing in the specified direction
- Front-left: 45° to your left
- Front-right: 45° to your right
- Back-left: 45° behind you to the left
- Back-right: 45° behind you to the right
"""
    else:
...
- Focus on the MOST RECENT image and connect it to previous images: How does this view relate to where you've been?
...
- Each image shows what you see from a specific position and orientation
- Compare consecutive images to understand how the space connects
- Use all images together to build a mental map of the room

For each image, note:
- Which objects do you see?
- Where are they relative to your current position?
- How does this compare to previous images?
...
1. **Current observation:** What do I see in the most recent image? What's in the foreground, left/right sides, background?
2. **Objects found so far:** Which objects from the question have I identified in ANY of the images?
3. **Missing information:** What objects/perspectives do I still need to see?
4. **Current opinion on answer:** Based on what I've seen, what's my best guess for the answer RIGHT NOW? Do I have enough confidence to answer?
5. **Next action:** Should I explore more (specify movement) or answer now?

**DECISION RULE:**
- If you have STRONG confidence in your answer → Set "done": true, provide "answer", NO movement needed
- If you need more exploration → Provide movement commands, set "done": false, answer can be null
- NEVER provide both movement AND done=true
...
**Important:**
- When done=true: all movement values MUST be 0
- When done=false: movement values can be non-zero
- Never provide movement commands when you already have an answer ready
...
def render_birds_eye_view_with_path(mesh: o3d.geometry.TriangleMesh, camera_positions: list, out_path: Path, marker_size=0.15):
    ...
    def create_filtered_mesh_for_view(view_type, view_direction=None):
        ...
        if view_type == "bev":
            ...
            if view_direction == "east":
                # Camera on +X side, remove +X wall
                ...
                # Camera on +Y side, remove +Y wall
    ...
    # For BEV: looking straight down, up vector determines rotation
    # For Dollhouse: looking at angle, up vector should give correct floor orientation
    views = [
        # BEV views - top-down
        ("bev_0", "bev", None,
        ...
        # For correct orientation: forward points toward room, up is perpendicular and gives floor-down
        ("dollhouse_east", "dollhouse", "east",
         np.array([x_max_full + dollhouse_distance, center_y, dollhouse_height]),
         None,  # Will compute forward to look at center
         None),
    ...
    for view_info in views:
        ...
        if forward is None:
            # Forward: from eye toward room center
            target = np.array([center_x, center_y, center_z])
            forward = target - eye
            forward = forward / np.linalg.norm(forward)
            
            # Up: ensure floor appears at bottom of image
            # We want the world +Z direction to point upward in the image
            world_up = np.array([0.0, 0.0, 1.0])
            
            # Compute right vector (horizontal in world coordinates)
            right = np.cross(forward, world_up)
            if np.linalg.norm(right) < 1e-6:
                # Forward is nearly vertical, use different reference
                right = np.array([1.0, 0.0, 0.0])
            right = right / np.linalg.norm(right)
            
            # Recompute up to be perpendicular to both forward and right
            up = np.cross(right, forward)
            
            # Ensure up points generally upward (positive Z component)
            if up[2] < 0:
                up = -up
        ...
        if "bev" in view_name:
            ...
            focal_length = 800.0  # Good balance for dollhouse
...
def run_single_question(mesh_path, question, choices, question_id, experiment_base_dir, scene_id, num_steps=NUM_STEPS, question_type="unknown", is_numerical=False, max_questions=None):
    ...
    for step in range(0, num_steps+1):
        ...
        # Build message with LABELED images - each image preceded by its step label
        content = []
        
        # Add images with labels first
        for img_idx, img_path in enumerate(image_history):
            if img_idx == 0:
                content.append({"type": "text", "text": f"\n**Image {img_idx} (Initial view):**"})
            else:
                content.append({"type": "text", "text": f"\n**Image {img_idx} (After movement {img_idx}):**"})
            content.append({"type": "image", "image": img_path})
        
        # Add the main prompt text after all images
        content.append({"type": "text", "text": f"\n\n{full_prompt}"})
    ...
    if enable_viz:
        ...
        try:
            print(f"[Q{question_id:03d}] 🗺️  Rendering trajectory visualization (question #{question_id})...")
            render_birds_eye_view_with_path(mesh, cam_history, birds_eye_path)
        except Exception as e:
            print(f"[WARN] Failed to render bird's eye view: {e}")
    else:
        print(f"[Q{question_id:03d}] ⏩ Skipping trajectory viz (only every 10th question for large runs)")
