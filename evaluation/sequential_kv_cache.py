#!/usr/bin/env python3
"""
Sequential pipeline with KV-Cache optimization for faster inference.

This version uses vLLM with prefix caching enabled to reuse KV cache
across multi-turn inference steps within each question.

Key optimization: enable_prefix_caching=True in vLLM allows reusing
the KV cache for shared prefixes (earlier images) when we add new images.

Usage:
  # Run with KV-cache optimization (vLLM only)
  python sequential_kv_cache.py --dataset combined --steps 15
"""
...
def main_sequential_split(mesh_base_dir=MESH_BASE_DIR, num_steps_per_question=NUM_STEPS, 
                          split=1, num_splits=1, continue_from=None, test_mode=False, max_questions=None, dataset="arkitscenes", question_types=None):
    """
...
def initialize_backend(backend_type: str = "vllm_kv"):
    """Initialize the global inference backend with KV-cache optimization."""
    global inference_backend
    
    print(f"\n[INFO] Initializing vLLM backend with KV-cache optimization...")
    
    # Import and use the KV-cache optimized backend
    from utils.inference_kv_cache import VLLMKVCacheBackend
    
    inference_backend = VLLMKVCacheBackend(
        model_id=MODEL_ID,
        cache_dir=CACHE_DIR,
    )
    print(f"[INFO] vLLM KV-Cache backend ready.\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run VSI-Bench with KV-Cache optimized vLLM")
    ...
    # Initialize the KV-cache optimized backend
    initialize_backend()
