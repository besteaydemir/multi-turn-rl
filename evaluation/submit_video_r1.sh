...
# Submit Video-R1 evaluation jobs for VSI-Bench
...
echo "======================================================================"
echo "VIDEO-R1 EVALUATION SUBMISSION"
echo "======================================================================"
...
#SBATCH --qos=mcml
#SBATCH --output=${LOG_DIR}/${TIMESTAMP}_vr1_download_%j.log
...
echo "Downloading Video-R1/Video-R1-7B model..."
python3 -c "
...
snapshot_download('Video-R1/Video-R1-7B', cache_dir=cache)
...
pattern = cache + '/models--Video-R1--Video-R1-7B/snapshots/*/preprocessor_config.json'
...
AutoProcessor.from_pretrained('Video-R1/Video-R1-7B')
print('Loading tokenizer...')
AutoTokenizer.from_pretrained('Video-R1/Video-R1-7B')
...
echo "  ✅ vr1_download: $DOWNLOAD_JOB_ID"
echo ""
...
submit_video_r1() {
    ...
    JOB_NAME="vr1_${NUM_FRAMES}f_s${SPLIT}"
    TIMESTAMP=$(date +%H-%M-%S)
...
echo ""
echo "✅ Video-R1 ${NUM_FRAMES} frames split ${SPLIT}/${NUM_SPLITS} complete"
EOF
)
...
# Save job list
JOB_LIST_FILE="${LOG_DIR}/video_r1_jobs.txt"
